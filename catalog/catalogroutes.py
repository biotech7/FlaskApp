...
from catalogDB import Base, Catalog, CatalogsItem, User
from flask import session as login_session
from flask_bcrypt import Bcrypt
from flaskForms import LoginForm, SignUpForm
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for("localsignin"))
    else:
...
def displayCatalog():
    displayCatalog = session.query(Catalog.cname).all()
    displayCatItems = (
        session.query(Catalog.cname, CatalogsItem.name)
...
def getCatalogItems(name):
    displayCatalogs = session.query(Catalog.cname).all()
    displayCatalog = (
        session.query(Catalog.cname).filter(Catalog.cname == name).first()
    )
    getItems = (
        session.query(Catalog, CatalogsItem)
...
def getCatalogDescription(getname, getdescname):
    getItem = session.query(CatalogsItem).filter(CatalogsItem.name == getdescname).all()
    if 'username' in login_session:
...
def editCatalog(editname):
    ...
    editItem = session.query(CatalogsItem).filter_by(name=editname).one()
    catalog = session.query(CatalogsItem).filter_by(id=editItem.user_id).one()
...
def deleteCatalog(deletename):
    if 'username' not in login_session:
        return redirect('/login')
    deleteItem = session.query(CatalogsItem).filter_by(name=deletename).one()
    catalog = session.query(CatalogsItem).filter_by(id=deleteItem.user_id).one()
    if login_session['user_id'] != catalog.user_id:
        return(
